...
const npmConfig = require("./helpers/getPackageConfig");
...
function deleteScripts() {
  return new Promise((resolve, reject) => {
    const delpath = del(["scripts"]);
    delpath.then(() => resolve()).catch((reason) => reject(new Error(reason)));
  });
}
  ...
  return new Promise((resolve, reject) => {
    exec("git remote -v", (err, stdout) => {
      ...
      const isClonedRepo = stdout
        ...
        .filter((line) => /POZAlabs\/poza-react-boilerplate\.git/.test(line))
        .length;
...
(async () => {
  ...
  await deleteScripts().catch((reason) => reportError(reason));
